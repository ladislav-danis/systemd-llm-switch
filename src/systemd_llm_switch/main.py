    ...
    def POST(self):
        try:
            ...
            if memories:
                ...
                memory_block = (
                    f"\n\n[PERSISTENT MEMORY]\n{memories}\n\n"
                    "CRITICAL INSTRUCTION: ALWAYS translate and store new "
                    "facts in ENGLISH language. Use 'save_memory' tool to "
                    "store ONLY NEW important facts about user preferences, "
                    "hardware, or project rules. Do not save information "
                    "that is already present in the memory block above. "
                    "Be concise."
                )
                if system_msg:
                    system_msg["content"] = \
