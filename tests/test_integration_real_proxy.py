...
@unittest.skipUnless(
    is_proxy_available(),
    f"Proxy not available at {PROXY_URL}"
)
class TestRealProxyIntegration(unittest.TestCase):
    ...
    def test_single_tool_call(self):
        ...
        self.assertEqual(tool_name, "get_weather")
    ...
    def test_parallel_tool_calls(self):
        ...
        payload = {
            ...
            "messages": [{
                "role": "user",
                "content": (
                    "Give me the weather and current time in Prague. "
                    "Use parallel calls for both functions."
                )
            }],
            "tools": self.tools,
            "tool_choice": "auto",
    ...
    def test_streaming_simulated_tool_call(self):
        """Test that streaming request returns valid tool calls."""
        payload = {
            "model": TEST_MODEL,
            "messages": [
        ...
        response = requests.post(
            self.url, json=payload, timeout=120, stream=True
        )
        self.assertEqual(response.status_code, 200)
        ...
        self.assertTrue(tool_calls_detected, "No tool calls in stream")
    ...
    def test_embeddings(self):
        ...
        embeddings_url = f"{PROXY_URL}/v1/embeddings"
        response = requests.post(embeddings_url, json=payload, timeout=120)
        self.assertEqual(response.status_code, 200)
